...
yData = S.Fit.YData;
xLim = S.Fit.Window;
ix = (xData >= xLim(1) & xData <= xLim(2)) ...
    & (xData < CLIP_RANGE(1) | xData > CLIP_RANGE(2));
...
% Axes.
pos = [0, 0, AXES_SIZE];
ax = axes('Position', pos, 'XLim', xLim, 'YLim', yLim, ...
...
h = Label.peak(S.Fit.Center(1, :), 'StringFormat', '%.1f', ...
    'FormatSpec', [2, ones(1, N - 1)], ...
    'FontColor', {[.3, .3, .3], [1, .5, 0]}, 'LineWidth', 0, ...
    'MinYPos', .1, 'MinYDist', .01, 'PlotLine', h, 'ClipRange', CLIP_RANGE);
