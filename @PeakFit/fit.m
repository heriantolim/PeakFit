function obj = fit(obj)
    ...
    %
    %  The fit results and statistics will be stored in the object properties
    %  after a successful calling of this method.
    %
    %  The object properties related to the start points and constraints for
    %  the fit parameters will also be initialized.
    %
    %  This algorithm is far from perfect. A lot of things need fixing to adapt
    %  to different scenarios or be more efficient.
    %
    % Tested on:
    %  - MATLAB R2013b
    %  - MATLAB R2015b
        ...
        if ~isfinite(height(1, j))
            ...
            if isfinite(height(3, j))
                ...
                if isfinite(height(2, j))
                    height(1, j) = height(2, j) ...
                        + HEIGHT_START * (height(3, j) - height(2, j));
                else
        ...
        if ~isfinite(width(1, j))
            ...
            if isfinite(width(3, j))
                ...
                if isfinite(width(2, j))
                    width(1, j) = width(2, j) ...
                        + WIDTH_START * (width(3, j) - width(2, j));
                else
        ...
        if ~isfinite(area(1, j))
            ...
            if isfinite(area(3, j))
                ...
                if isfinite(area(2, j))
                    area(1, j) = area(2, j) ...
                        + HEIGHT_START * WIDTH_START * (area(3, j) - area(2, j));
                else
        ...
        for j = 1:np
            ...
            if isfinite(area(i, j))
                ...
                if ~isfinite(width(i, j)) && isfinite(height(n(i), j))
                    width(i, j) = PeakFit.computewidth(ps(j), ...
                        area(i, j), height(n(i), j));
                elseif ~isfinite(height(i, j)) && isfinite(width(n(i), j))

                    if area(i, j) < 0
                        height(i, j) = PeakFit.computeheight(ps(j), ...
                            area(n(i), j), width(i, j));
                    else
                        height(i, j) = PeakFit.computeheight(ps(j), ...
                            area(i, j), width(n(i), j));
                    end
    ...
    for j = 1:np
        ...
        if height(1, j) < 0
            ...
            if ~isfinite(height(2, j))
                height(2, j) = -HEIGHT_UP ...
                    * min(y(x >= center(2, j) & x <= center(3, j)));
            end
            ...
            if ~isfinite(height(3, j))
                height(3, j) = HEIGHT_UP ...
                    * max(y(x >= center(2, j) & x <= center(3, j)));
            end
    ...
    for i = 1:3
        ...
        for j = 1:np
            ...
            if ~isfinite(area(i, j))
                area(i, j) = PeakFit.computearea(ps(j), ...
                    height(i, j), width(i, j));
            end
