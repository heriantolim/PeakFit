classdef PeakFit
    %% PeakFit Class
    %  Fit a spectral curve with a linear combination of symmetric peak
    %  functions such as Gaussian, Lorentzian, etc. A baseline polynomial
    %  function may be included to represent a 'background' contribution to the
    %  spectra.
    %
    % Constructions:
    %  A PeakFit object can be constructed in two ways.
    %
    %  The first is by creating an empty PeakFit object and specifying the
    %  fitting parameters via property assignments as follows:
    %
    ...
    %
    %  To perform the peak fitting, call another PeakFit with the prepared obj
    ...
    %  disp(obj);
    %
    %  The second is by specifying the data points and fitting parameters
    %  together in one call as follows:
    %
    %  obj = PeakFit(Data, ...
    %      Window, [...], ...
    %      CenterLow, [...], ...
    ...
    %  CenterUp, WidthUp, and BaselinePolyOrder, would improve the accuracy of
    %  the peak fitting significantly.
    %
    % Public Properties:
    %  Data: The data points of the curve. An alternative to XData and YData.
    %        Data must be a two-column or two-row matrix.
    %
    %  XData: The X data points of the curve.
    %
    %  YData: The Y data points of the curve. The peak fitting may not work
    %         properly if some Y data points are negative.
    %
    ...
    %            shape, start point, lower, or upper bound are set with vectors
    %            of length greater than NumPeaks, then NumPeaks will be
    %            incremented to adjust to the maximum length of these vectors.
    %            When the maximum length of these vectors is less, then these
    %            vectors will be expanded and filled with the default values.
    %            When NumPeaks=0 and all the start point, lower, and upper are
    %            not set, then the program attempts to fit all peaks found in
    %            the curve.
    %
    ...
    %             'Gaussian' (2). PeakShape may be set with an integer, the
    %             initial of these names, e.g. 'L' or 'G', or the full name,
    %             e.g. 'Lorentzian'. When the length of PeakShape is less than
    %             NumPeaks, the remaining peaks will have a default PeakShape,
    %             which is 'Lorentzian'. If PeakShape contains only one
    %             element, then the default value is the same as that element.
    %
    %  BaselinePolyOrder: An integer that specifies the order of the polynomial
    %                     function used to fit the background of the spectrum.
    %                     Defaults to 0 (a constant polynomial). Set this to a
    %                     negative value to exclude the polynomial from the fit
    %                     model.
    %
    %  ...Start: A vector of initial values for the ... coefficients, where the
    %            blanks are one of the {Area, Center, Width, Height, Baseline}.
    %            The default values are determined heuristically. To make
    %            certain properties default, set their values to NaN. They will
    %            be then replaced with the default values upon fitting.
    %
    %  ...Low: A vector of lower bounds for the ... coefficients, where the
    %          blanks are one of the {Area, Center, Width, Height, Baseline}.
    %          The default values are determined heuristically. To make certain
    %          properties default, set their values to -Inf. They will be then
    %          replaced with the default values upon fitting.
    %
    %  ...Up: A vector of upper bounds for the ... coefficients, where the
    %         blanks are one of the {Area, Center, Width, Height, Baseline}.
    %         The default values are determined heuristically. To make certain
    %         properties default, set their values to Inf. They will be then
    %         replaced with the default values upon fitting.
    %
    %  Robust: The type of the least-squares method to be used in the fitting.
    %          Available values are 'off', 'LAR' (least absolute residual), and
    %          'Bisquare' (bisquare weights). Defaults to 'off'.
    %
    %  Algorithm: The algorithm to be used in the fitting. Available values are
    %             'Lavenberg-Marquardt', 'Gauss-Newton', or 'Trust-Region'.
    %             Defaults to 'Trust-Region'.
    %
    %  MovMeanWidth: The window width of the moving average used for smoothing
    %                the curve in order to filter out the noise before finding
    %                the maximas. This parameter is used only when CenterStart
    %                is not given. The value of this can be set as a positive
    %                integer which specifies the width in terms of the number
    %                of data points, OR a real scalar between [0,1] which
    %                specifies the width as a fraction of the total number of
    %                data points.
    %
    ...
    %
    %  MaxFunEvals: The allowed maximum number of evaluations of the model.
    %               Defaults to 1e5.
    %
    %  MaxIters: The maximum number of iterations allowed for the fit. Defaults
    %            to 1e3.
    %
    %  TolFun: The termination tolerance on the model value. Defaults to 10^-6.
    %
    %  TolX: The termination tolerance on the coefficient values. Defaults to
    %        10^-6.
    %
    % Read-only Properties:
    %  Method: The method used for the fitting, which is
    ...
    %
    %  Area, Center, Height, Width, Baseline: A 3-by-NumPeaks matrix that
    %        stores the fit results for the area,... , respectively. The first
    %        row is the values at convergence; the second row is the 95% CI
    %        lower bounds; and the third row is the 95% CI upper bounds.
    %
    %  RelStDev: The relative standard deviation of the fit results.
    %
    %  CoeffDeterm: The coefficient of determination of the fit results.
    %
    %  AdjCoeffDeterm: The degree-of-freedom adjusted coefficient of
    %                  determination of the fit results.
    %
    %  NumFunEvals: The number of function evaluations.
    %
    ...
    %            maximum number of function evaluations or iterations was
    %            exceeded. Negative flags indicate that the algorithm did not
    %            converge to a solution.
    %
    % Public Methods:
    %  disp: Displays the options, results, error and performance of the
    %        fitting.
    %
    %  model: Returns the reconstructed data points (model) using the fit
    %         results.
    %
    %  convertunit: Converts the units of the data points and the fit results.
    %               Available units to be converted from or to are:
    %                'eV'        : electron volt
    %                'percm'     : cm^{-1} (wavenumber)
    %                'Ramanshift': cm^{-1} (Raman shift)
    %               When converting to or from Raman shift, an additional
    %               argument is required that specifies the excitation
    ...
    methods
        ...
        function x = get.Area(obj)
            ...
            if isempty(peakShape) || isempty(height) || isempty(width)
                ...
                for i = 1:numPeaks
                    x(:, i) = PeakFit.computearea(peakShape(i), ...
                        height(:, i), width(:, i));
                end
        ...
        function obj = set.WidthStart(obj, x)
            ...
            elseif isrealvector(x) && all(isnan(x) | x >= 0)
                ...
                error('PeakFit:setWidthStart:InvalidInput', ...
                    'Input to set the WidthStart must be a vector of positive real numbers.');
            end
        ...
        function obj = set.WidthLow(obj, x)
            ...
            elseif isrealvector(x) && all(isnan(x) | x >= 0)
                ...
                error('PeakFit:setWidthLow:InvalidInput', ...
                    'Input to set the WidthLow must be a vector of positive real numbers.');
            end
        ...
        function obj = set.WidthUp(obj, x)
            ...
            elseif isrealvector(x) && all(isnan(x) | x >= 0)
                ...
                error('PeakFit:setWidthUp:InvalidInput', ...
                    'Input to set the WidthUp must be a vector of positive real numbers.');
            end
        ...
        function obj = set.MovMeanWidth(obj, x)
            ME = MException('PeakFit:setMovMeanWidth:InvalidInput', ...
                'Input to set the MovMeanWidth must be a positive integer scalar or a real scalar between [0,1].');
