...
%
%  YData: The Y data points of the curve. The peak fitting may not work properly
%         if some Y data points are negative.
%
...
%            blanks are one of the {Area, Center, Width, Height, Baseline}. The
%            default values are determined heuristically. To make certain
%            properties default, set their values to NaN. They will be then
%            replaced with the default values upon fitting.
%
%  ...Low: A vector of lower bounds for the ... coefficients, where the blanks
%          are one of the {Area, Center, Width, Height, Baseline}. The default
%          values are determined heuristically. To make certain properties
%          default, set their values to -Inf. They will be then replaced with
%          the default values upon fitting.
%
%  ...Up: A vector of upper bounds for the ... coefficients, where the blanks
%         are one of the {Area, Center, Width, Height, Baseline}. The default
%         values are determined heuristically. To make certain properties
%         default, set their values to Inf. They will be then replaced with the
%         default values upon fitting.
%
%  Robust: The type of the least-squares method to be used in the fitting.
%          Available values are 'off', 'LAR' (least absolute residual method),
%          and 'Bisquare' (bisquare weights method). Defaults to 'off'.
%
%  Algorithm: The algorithm to be used in the fitting. Available values are
%             'Lavenberg-Marquardt', 'Gauss-Newton', or 'Trust-Region'. Defaults
%             to 'Trust-Region'.
%
%  MovMeanWidth: The window width of the moving average used for smoothing the
%                curve in order to filter out the noise before finding the
%                maximas. This parameter is used only when CenterStart is not
%                given. The value of this can be set as a positive integer which
%                specifies the width in terms of the number of data points, OR a
%                real scalar between [0,1] which specifies the width as a
...
%
%  Area, Center, Height, Width, Baseline: A 3-by-NumPeaks matrix that stores the
%        fit results for the area,... , respectively. The first row is the
%        values at convergence; the second row is the 95% CI lower bounds; and
%        the third row is the 95% CI upper bounds.
%
...
%
%  model: Returns the reconstructed data points (model) using the fit results.
%
