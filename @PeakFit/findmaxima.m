function [xm, ym] = findmaxima(obj, varargin)
    %% Find Maxima
    %  [xm, ym] = obj.findmaxima() locates the maxima in the curve data stored
    %  in the object and returns the coordinates of the maxima.
    %
    %  [xm, ym] = obj.findmaxima(window) locates the maxima within the
    %  specified x window. window is a real vector of length two.
    %
    %  [xm, ym] = obj.findmaxima(n) locates exactly n maxima.
    %
    %  [xm, ym] = obj.findmaxima(window, n) locates n maxima within the window.
    %
    %  [xm, ym] = obj.findmaxima(x, y, ...) use the x and y vector as the data
    %  points instead of the data stored in the object.
    %
    %  The curve is smoothed first prior to finding the maxima with a simple
    %  moving mean. The MovMeanWidth property is used as the sample
    %  width of the moving mean.
    %
    % Requires package:
    %  - MatCommon_v1.0.0+
    %
